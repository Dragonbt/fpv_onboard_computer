...
vector<float> pos_ne2xy(vector<float>pos_ne, float yaw_rad) {
...
vector<float> pos_xy2ne(vector<float>pos_xy, float yaw_rad) {
...
vector<float> UpdateLocalvel(shared_ptr<Telemetry> telemetry ) {
	vector<float> vel = { 0.0f,0.0f,0.0f };
	Telemetry::PositionVelocityNED position_velocity_ned = telemetry->position_velocity_ned();
	vel[0] = position_velocity_ned.velocity.north_m_s;
	vel[1] = position_velocity_ned.velocity.east_m_s;
	vel[2] = position_velocity_ned.velocity.down_m_s;
	return vel;
}
vector<float> UpdateLocalpos(shared_ptr<Telemetry> telemetry) {
	vector<float> pos = { 0.0f,0.0f,0.0f };
	Telemetry::PositionVelocityNED position_velocity_ned = telemetry->position_velocity_ned();
	pos[0] = position_velocity_ned.position.north_m;
	pos[1] = position_velocity_ned.position.east_m;
	pos[2] = position_velocity_ned.position.down_m;
	return pos;
}
float altitudeThrustControl( float _pos_sp_z, shared_ptr<Telemetry> telemetry, float dt )
{
	...
	offset_thrust = mid_thrust / (cos(deg2rad(_pitch))*cos(deg2rad(_roll)));
	//std::cout << "pitch=" << _pitch << "roll=" << _roll << endl;
    if (_pos_z < limit_pos_z || landing_flag) {
		std::cout << "---Reject PID control,Start land---" << endl;
		...
		Times++;
		landing_flag = 1;
        //std::cout << "pos_z: " << _pos_z << endl;
	}
    else{
		down_reference_topic.update(_pos_sp_z);
        float err_pos_z;
		...
		thrust = -thrust;
		//std::cout << "pos_sp_z: " << _pos_sp_z << " pos_z: " << _pos_z << " vel_z: " << _vel_z << " _int_pos_z: " << _int_pos_z << " thrust: " << thrust << endl;
    }
    return thrust;
}
...
vector<float> positionThrustControl(vector<float> _pos_sp, shared_ptr<Telemetry> telemetry, float dt) {
	...
	vector<float> pos_sp = { _pos_sp[0],_pos_sp[1] };
	...
	vector<float> _pos_err_ne = { 0.0f,0.0f };
	float thrust_max = 1.0f;
	Telemetry::PositionVelocityNED position_velocity_ned = telemetry->position_velocity_ned();
	Telemetry::EulerAngle euler_angle = telemetry->attitude_euler_angle();
	float yaw = deg2rad(euler_angle.yaw_deg);
	...
	if (flow_effective) {
		...
		_pos_err_ne = pos_sp - _pos;
		//_pos_err_n = 1.0f;
		//_pos_err_e = 1.0f;
		_pos_err = pos_ne2xy(_pos_err_ne, yaw);
		_pos_err[0] = limit_values(_pos_err[0], -0.5f, 0.5f);
		_pos_err[1] = limit_values(_pos_err[1], -0.5f, 0.5f);
		//std::cout << "_pos_err_x:" << _pos_err[0] << endl;
		//std::cout << "_pos_err_y:" << _pos_err[1] << endl;
	}
	else if (vision_effective) {
		float _yaw = deg2rad(euler_angle.yaw_deg);
		_pos[0] = position_velocity_ned.position.north_m;
		_pos[1] = position_velocity_ned.position.east_m;
		...
		//std::cout << "_pos_err_x_from_flow = " << my_pos_sp_x << " " << "_pos_err_x_from_vision = " << _pos_sp[0] << endl;
	...
	if (position_hold_xy) {//state=take_off||state=landing
		...
		if ((thrust_desired_NE[0] * thrust_desired_NE[0] + thrust_desired_NE[1] * thrust_desired_NE[1]) > thrust_max_NE * thrust_max_NE) {
			...
			_thr_sp = thrust_desired_NE / mag * thrust_max_NE;			
		}
...
void altitudeTest( shared_ptr<Telemetry> telemetry, shared_ptr<Offboard> offboard, float _Kp_z, float _Ki_z, float _Kd_z,float _Kp_x, float _Ki_x, float _Kd_x,float _Kp_y, float _Ki_y, float _Kd_y )
{
	...
	vector<float> _pos_sp_pre = { 0.0f, 0.0f, 0.0f };
	//vector<float> _vel = { 0.0f, 0.0f, 0.0f };
	//vector<float> _vel_pre = { 0.0f, 0.0f, 0.0f };
	//vector<float> _pos = { 0.0f, 0.0f, 0.0f };
	//vector<float> _pos_pre = { 0.0f, 0.0f, 0.0f };
	vector<float> update_pos_sp = { 0.0f, 0.0f, 0.0f };
	float yaw = 0;
	float thrust = 0;
	...
	float len;
	vector<float> off_xyz = { 0.0f,0.0f,0.0f };
	vector<float> off_ned = { 0.0f,0.0f,0.0f };
	vector<float> att_sp = {0.0f, 0.0f};
	high_resolution_clock::time_point last_time = high_resolution_clock::now();
	high_resolution_clock::time_point last_time_pre;
	Kp_z = _Kp_z;
	Ki_z = _Ki_z;
	Kd_z = _Kd_z;
	...
	Ki_y = _Ki_y;
	Kd_y = _Kd_y;
	std::cout << "Kp_z:" << Kp_z << endl;
	std::cout << "Ki_z:" << Ki_z << endl;
	std::cout << "Kd_z:" << Kd_z << endl;

	std::cout << "Kp_x:" << Kp_x << endl;
	std::cout << "Ki_x:" << Ki_x << endl;
	std::cout << "Kd_x:" << Kd_x << endl;

	std::cout << "Kp_y:" << Kp_y << endl;
	std::cout << "Ki_y:" << Ki_y << endl;
	std::cout << "Kd_y:" << Kd_y << endl;
    ...
    while(true)
    {
		...
		switch( status )
		{
			...
			case SAFE_QUIT_COMMAND:
				landing_flag = 1;
				status = 1;
				remotePrint("auto landing");
				break;
			case FORCE_QUIT_COMMAND:
				quitOffboard( offboard );
				remotePrint("quit mission");
				return;
			case STEP_COMMAND:
				step = param * -1;
			...
			case 1:
				...
				t0 = high_resolution_clock::now();
				//control code
				//std::cout << "Kp_z: " << Kp_z << " Ki_z: "  << Ki_z << " Kd_z: " << Kd_z << endl; 
			...
			case FLOW_HOLD_COMMAND:
				...
				std::cout << param << endl;
				_pos_sp = UpdateLocalpos(telemetry);
				_pos_sp[0] +=  param;
				...
				_thr_sp[0] = limit_values(_thr_sp[0], -0.707f * _thr_sp[2], 0.707f * _thr_sp[2]);
				_thr_sp[1] = limit_values(_thr_sp[1], -0.707f * _thr_sp[2], 0.707f * _thr_sp[2]);
				//std::cout << "_thr_sp[0] = " << _thr_sp[0] << " " << "_thr_sp[1] = " << _thr_sp[1] << " " << "_thr_sp[2] = " << _thr_sp[2] << endl;
				...
				//std::cout << "roll" << asinf(_thr_sp[1] / _thr_sp[2]) << endl;
				//std::cout << "pitch" << asinf(_thr_sp[0] / _thr_sp[2]) << endl;
				attitude = {rad2deg(_roll_sp), rad2deg(_pitch_sp), yaw, _thr_sp[2]};
				//std::cout << attitude << endl;
				offbCtrlAttitude(offboard, attitude);
				status = 3;
				break;
			case 3:
			...
			case VISION_HOLD_COMMAND:
				...
				//position_velocity_ned = telemetry->position_velocity_ned();
				if( ! target_topic.latest(timestamp, target) )
					target.confidence = -1;
				if( target.confidence > 0 &&  timestampf() - timestamp < 500)
				{
					last_time = high_resolution_clock::now();
					yaw_rad = deg2rad(yaw);
					off_xyz[0] = target.z_m - 4.0f;
					off_xyz[1] = target.x_m;
					off_xyz[2] = target.y_m;
					off_ned[0] = off_xyz[0] * cos(yaw_rad) - off_xyz[1] * sin(yaw_rad);
					off_ned[1] = off_xyz[0] * sin(yaw_rad) + off_xyz[1] * cos(yaw_rad);
					off_ned[2] = off_xyz[2];
					
					/*
					//Credibility test
					_vel = UpdateLocalvel(telemetry);
					_pos = UpdateLocalpos(telemetry);
					intervalMs(high_resolution_clock::now(), last_time)/1000
					*/
					...
					/*
					if (_pos_sp[0] == 0 && _pos_sp[1] == 0 && _pos_sp[2] == 0) {
						_pos_sp = UpdateLocalpos(telemetry);
					}
					*/
					if (off_xyz[0] == -4.0f && off_xyz[1] == 0.0f && off_xyz[2] == 0.0f) {
						_pos_sp = _pos_sp_pre;
					}
					else {
						update_pos_sp = UpdateLocalpos(telemetry);
						//Single channel test
						//_pos_sp[0] = update_pos_sp[0] + off_ned[0];
						//_pos_sp[1] = update_pos_sp[1] + off_ned[1];
						_pos_sp[2] = update_pos_sp[2] + off_ned[2];
						//last_time_pre = last_time;
						//_vel_pre = _vel;
						//_pos_pre = _pos;
						//off_xyz_pre = off_xyz;
					}
				...
				attitude = {rad2deg(_roll_sp), rad2deg(_pitch_sp), yaw, _thr_sp[2]};
				//std::cout << attitude << endl;
				//std::cout << "real yaw: " << telemetry->attitude_euler_angle().yaw_deg << endl;
				offbCtrlAttitude(offboard, attitude);
				break;
			/*
			case VISION_HOLD_COMMAND:
				...
				if( target.confidence > 0 && time_ms - target.time_ms < 100)
				{
					...
					std::cout << "[WARNING]: " << "VISION FAIL " << target.confidence << " " << time_ms - target.time_ms << endl;
					break;
				}
...
void altitude(shared_ptr<Telemetry> telemetry,shared_ptr<Offboard> offboard, double dt, float P, float I, float D) {
    ...
    std::cout << Kp_z << " " << Kd_z << endl;
    std::cout << _pos_z << " " << limit_pos_z << " " << flag_project << endl;
	...
	//single-loop PID
		...
		//start PID control
		std::cout << "---START PID control---" << endl;
