...
double pre_vel_err_z=0.0,  _vel_sp_z=0.0, _thr_int_z=0.0, _pos_sp_z;
float yaw, limit_pos_z, _int_pos_z = 0.0f;
int SampleTime = 20;
int Times = 0;
bool flag_project = 0;
    ...
    while(true)
    {
        ...
        if( command.index == 0 )
        {
            ...
            _pos_sp_z = position.position.down_m;
             _vel_sp_z = position.velocity.down_m_s;
...
void altitudeDouble(shared_ptr<Telemetry> telemetry, shared_ptr<Offboard> offboard, double dt)
{
	...
	double Kp_z = 1.0;
	double Kp_Vz = 0.2, Ki_Vz = 0.02, Kd_Vz = 0.0;
    position = telemetry->position_velocity_ned();
    ...
    if (_pos_z < limit_pos_z || flag_project) {
        ...
        myPID_z.Compute();*/
        const double pos_err_z = _pos_sp_z - _pos_z;
        vel_sp_pos_z = Kp_z * pos_err_z;
        ...
        thrust = thrust_desired_D > uMin ? (thrust_desired_D < uMax ? thrust_desired_D : uMax) : uMin;
        thrust = -thrust;
        cout << "pos_sp_z: " << _pos_sp_z << " pos_z: " << _pos_z << " vel_sp_pos_z: " << vel_sp_pos_z << "_thr_int_z:" << _thr_int_z << endl;
